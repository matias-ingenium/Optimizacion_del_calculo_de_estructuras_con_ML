...
def training_data(n_snapshots, mesh_file, facet_file, physical_file):
    ...
    snapshot_data_file = f"generando_matriz_inversa/elementos de prueba/datos_entrenamiento_simplificado_{n_snapshots}.pkl"
    ...
    if os.path.exists(snapshot_data_file):
        ...
        training_set = [tuple(random.uniform(-1, 1) for _ in range(4)) for _ in range(n_snapshots)]
